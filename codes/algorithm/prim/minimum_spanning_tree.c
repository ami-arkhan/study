...
int V, E, heapSize, edges, minWeight;
...
// Priority queue (Heap) started from '0' index
struct heapNode {
	int vertex;
	int parent;
	int weight;
...
void readCase()
{
	...
	for (i = 0; i < E; i++)
	{
		...
		************************************************************************/
		...
		makeAdjList(dest, src, weight);
	}
}

void mst_prim(int vertex)
{
	struct node *cur;
	struct heapNode *temp;
	...
	// Run BFS till '(V - 1)' edges are found to generate MST
	edges = -1;
	while (edges != (V - 1))
	{
		...
		// Minimum weighted adjustened vertex is added and edge count increased
		...
		// For debugging which vertex are connecting to the tree
		//printf("%d %d %d\n", temp->parent, temp->vertex, temp->weight);

		/****************************************************************************
		*	1. Prim's Algorithm states that minimum vertex is added	from			*
		*		'unvisited vertex' that has an edge to current constructing tree.	*
		*	2. In heap only possilbe unvisited vertex are inserted by BFS,			*
		*		that is which have connection to current constructing tree.			*
		*	3. As BFS is applied for pushing, there is no chance for pushing		*
...
int main()
{
	...
	for (test = 1; test <= T; test++)
	{
		...
		// Prim's Algorithm to find MST (Minimum Spanning Tree) from source vertex: 1
		mst_prim(1);
